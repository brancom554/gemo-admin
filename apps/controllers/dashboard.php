...
if ($url_array[2] == "barChart") {
    $moiss = $totall = array();
    $db = new Database();
    $sql5 = 'SELECT MONTHNAME(operation_date) as mois,count(operation_id) as total FROM operations WHERE operations.company_token="'.$_SESSION['companie_token'].'" GROUP BY month(operation_date) ORDER BY month(operation_date) ASC';
    ...
    echo json_encode(['mois' => $moiss,'total' => $totall]);
    exit;
}
